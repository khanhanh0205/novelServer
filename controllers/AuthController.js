...
export const AuthController = {
    ...
    RegisterUser: async (req, res) => {
        try {
            ...
            // sendMail(req.body.email, "Kích hoạt tài khoản", config.CLIENT_URL + "active/" + activeCode)
            console.log( config.CLIENT_URL + "active/" + activeCode)
    ...
    RefreshToken: async (req, res) => {
        try {
            ...
            jwt.verify(refreshToken, config.JWT_ACCESS_KEY, (err, user) => {
                if (err) {
                    console.log("Lỗi:" + err)
        ...
        try {
            ...
            if (email) {
                ...
                if (user) {
                    ...
                    sendMail(email, "Kích hoạt tài khoản", config.CLIENT_URL + "active/" + activeCode)
                        .then(response => {
    ...
    Active: async (req, res) => {
        try {
            ...
            if (key) {
                jwt.verify(key, config.JWT_ACCESS_KEY, async (err, user) => {
                    if (err) {
                        console.log(err)
